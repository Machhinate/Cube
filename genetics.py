...
def individualise(number_of_genes, upper_limit, lower_limit):
...
def population(number_of_individuals, number_of_genes, upper_limit, lower_limit):
    return [individualise(number_of_genes, upper_limit, lower_limit) for x in range(number_of_individuals)]
...
def selection(generation, method='Fittest Half'):
    ...
    if method == 'Roulette Wheel':
        ...
        selected_fitness = [generation['Fitness'][-x-1] for x in range(int(len(generation['Individuals'])//2))]
        selected = {'Individuals': selected_individuals, 'Fitness': selected_fitness}

    elif method == 'Random':
        selected_individuals = [generation['Individuals'][randint(1, len(generation['Fitness']))]
                                for x in range(int(len(generation['Individuals'])//2))]
        selected_fitness = [generation['Fitness'][-x-1]
                            for x in range(int(len(generation['Individuals'])//2))]
        selected = {'Individuals': selected_individuals, 'Fitness': selected_fitness}
...
def mating(parents, method='Single Point'):
    ...
    if method == 'Two Points':
        ...
        if pivot_point_2 < pivot_point_1:
            pivot_point_2, pivot_point_1 = pivot_point_1, pivot_point_2
        offsprings = \
            [parents[0][:pivot_point_1] + parents[1][pivot_point_1: pivot_point_2] + parents[0][pivot_point_2:],
             parents[1][:pivot_point_1] + parents[0][pivot_point_1: pivot_point_2] + parents[1][pivot_point_2:]]
...
def next_generation(gen, upper_limit, lower_limit):
    ...
    unmutated = selected['Individuals']+offsprings2+offsprings1
    mutated = [mutation(unmutated[x], upper_limit, lower_limit)
               for x in range(len(unmutated['Individuals'])-1)]
    ...
    next_gen['Fitness'] = [sorted_next_gen[x][1]
                           for x in range(len(sorted_next_gen))]
    next_gen['Individuals'].append(elit['Individuals'])
    next_gen['Fitness'].append(elit['Fitness'])
    return next_gen
...
while not finish:
    ...
    if max(fitness_avg) > 5:
        break
    if fitness_similarity_chech(fitness_max, 50):
        break
